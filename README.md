...
<img title="" src="file:///D:/aicode/dsh/voice-agent/media/a68961b20a78ae784af05c8ec2bc6fdc50b759e9.png" alt="img_3240_1b8423c892d49b539827ba18c6197a96.png" width="192">
...
| 🧪 离线可验证  | 12 个测试脚本、420 项断言；含鼠标键盘真窗口点击打字、工具层全量体检、权限闸门，不接麦克风也能跑完                          |
...
**不用手写 YAML**：设置 → 大脑（LLM）→ **配置多模型…** 里能直接改。上面一排是路由
（哪个用途用哪个档案），下面是档案卡片（名称 / 模型 / 地址 / 密钥 / 思考程度 / 温度 /
是否会看图），带「添加档案」和「删除」。存完立刻生效 —— 背后会把已经建好的模型客户端
丢掉重建，不用重启程序。
...
十二个测试脚本共 **420 项断言全部通过**（另加 selftest 10 项）：
...
| test_security | 46 | 权限闸门：模式阶梯、fail closed、限流、明文降级、审计 |
...
│  │   ├─ models_dialog.py 多模型配置：路由 + 档案卡片
...
├─ tests/                十二个测试脚本（run_tests.py 一把跑完）
│                       含 test_input（真窗口点击打字）、test_tools（工具层全量体检）、
│                       test_watch（轮询）、test_subagent（子代理）
├─ config.example.yaml
└─ requirements.txt
```
