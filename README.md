...
| 🎯 屏幕标记   | 「框一下这块」→ 记成范围1、范围2（主题色虚线框）；「标在这儿」→ 点1、点2（半透明圆点）。之后说「看看范围1」就行                    |
...
| 🧪 离线可验证  | 14 个测试脚本、536 项断言；含鼠标键盘真窗口点击打字、工具层全量体检、权限闸门、屏幕标记，不接麦克风也能跑完                       |
...
不会越框越多）。界面菜单里也有「框选范围」「标记点」「擦掉所有标记」三个入口。
...
十四个测试脚本共 **536 项断言全部通过**（另加 selftest 10 项）：
