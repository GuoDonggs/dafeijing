...
- [十二、命令行](#十二命令行)
- [十三、配置文件详解](#十三配置文件详解)
- [十四、权限：别让模型（和中转站）碰你的电脑](#十四权限别让模型和中转站碰你的电脑)
- [十五、关键设计取舍](#十五关键设计取舍)
- [十六、实测数据](#十六实测数据)
- [十七、打包成 exe](#十七打包成-exe)
- [十八、常见问题](#十八常见问题)
- [十九、目录结构](#十九目录结构)
- [二十、已知限制](#二十已知限制)
...
| 🧪 离线可验证  | 14 个测试脚本、515 项断言；含鼠标键盘真窗口点击打字、工具层全量体检、权限闸门、屏幕标记，不接麦克风也能跑完            |
...
十四个测试脚本共 **515 项断言全部通过**（另加 selftest 10 项）：

| 脚本                         | 项数      | 覆盖什么                             |
| -------------------------- | ------- | -------------------------------- |
| selftest                   | 10      | 端到端自检：模型、合成→识别回环、工具、大脑           |
| test_pipeline              | 39      | 状态机：唤醒→识别→执行→播报→打断→确认→超时→长指令     |
...
├─ tests/                十四个测试脚本（run_tests.py 一把跑完）
