...
- [二、快速开始](#二快速开始)
- [三、界面](#三界面)
  - [输出音量](#输出音量) · [主题色](#主题色) · [改了要重启的设置？主界面会告诉你](#改了要重启的设置主界面会告诉你)
  - [过渡动效一览](#过渡动效一览) · [网页版](#网页版可选)
- [四、语音链路：算力与音质怎么选](#四语音链路算力与音质怎么选)
  - [音色：Kokoro 的 0 号是个大坑](#音色kokoro-的-0-号是个大坑)
  - [声音不好听，往往是这三件事](#声音不好听往往是这三件事)
  - [Kokoro 怎么调才不拖沓](#kokoro-怎么调才不拖沓)
- [五、多模型：不同用途挂不同的模型](#五多模型不同用途挂不同的模型)
- [六、工具清单](#六工具清单)
- [七、自定义技能](#七自定义技能)
- [八、声纹：只认你的声音](#八声纹只认你的声音)
- [九、实战：找图 + 连点器](#九实战找图--连点器)
- [十、命令行](#十命令行)
- [十一、配置文件详解](#十一配置文件详解)
- [十二、关键设计取舍](#十二关键设计取舍)
- [十三、实测数据](#十三实测数据)
- [十四、打包成 exe](#十四打包成-exe)
- [十五、常见问题](#十五常见问题)
- [十六、目录结构](#十六目录结构)
- [十七、已知限制](#十七已知限制)
...
| 🔊 本地语音合成 | VITS（快）或 Kokoro（103 种音色、更自然）二选一，长回复按句流式播放 |
| ⚙️ 算力可选 | CPU / GPU 自动或手动，三档资源占用预设，界面里直接切 |
| 🧠 多模型 | 主对话、同意判定、看图可以分别挂不同的模型和思考深度 |
| 🪟 电脑控制 | 33 个内置工具：应用、网址、搜索、音量、媒体、截图、剪贴板、文件、打字、快捷键、窗口、锁屏、电源、命令、记忆 |
...
| quality | 8 | Kokoro | 想要更自然的音色，机器也够快 |

单独写 speech.threads 或 tts.engine 就按你写的来，档位只提供默认值。

### 关于 GPU：先说清楚一件事

speech.device: cuda 会**如实告诉你最终跑在哪里**。原因是有个坑：

> **sherpa-onnx 官方 pip 包是 CPU-only 构建。** 即使你装了 onnxruntime-gpu、
> 机器上有显卡，它也会打印一句 "Please compile with -DSHERPA_ONNX_ENABLE_GPU=ON"
> 然后悄悄退回 CPU。

所以 provider 检测会同时看两件事：onnxruntime 有没有 CUDA、sherpa-onnx
这个安装包本身有没有把 GPU 编进去。只要有一项不满足，doctor 和界面就写
...
| 引擎 | 加载 | 合成耗时 | RTF | 音色数 |
| --- | --- | --- | --- | --- |
| VITS | 0.8s | 3.9 秒音频用 1.04s | 0.27 | 5 |
| Kokoro | 1.9s | 7.9 秒音频用 7.43s | 0.94 | **103** |

Kokoro 好听得多，但慢了 3~4 倍，几乎贴着实时跑。所以默认给 VITS，
想要音质再自己切 quality 档或把 tts.engine 设成 kokoro。

### 音色：Kokoro 的 0 号是个大坑

**Kokoro 的 0~2 号是英文音色**（0 = af_maple，1 = af_sol，2 = bf_vale），
3~57 才是中文女声（zf_*），58~102 是中文男声（zm_*）。
拿英文音色的风格向量去念中文，音素对不上，出来就是**发闷、发粗、带明显的电流声**，
而且语气很怪 —— 听感上像另一个人在念稿。

这不是猜的。把 103 个音色逐个合成同一句话、测频谱（scripts/scan_kokoro_voices.py）：

| 音色 | 谱平坦度 | 6kHz 以上能量 | 帧间抖动 |
| --- | --- | --- | --- |
| af_maple（0 号，英文） | 0.0214 | **0.0897** | **0.964** |
| 中文女声中位数 | 0.0155 | 0.0462 | — |
| zf_003（现在的默认） | **0.0070** | **0.0181** | **0.144** |

高频能量差 5 倍、抖动差 6 倍 —— 那阵"电流声"就是这么来的。
所以代码里做了一层保护：**配置写了 0~2 号会被自动换成中文音色**，
启动日志里写明换成了哪个（voice_agent/voices.py 的 sanitize）。

换音色按名字写，别写编号：

```powershell
python -m voice_agent voices                        # 列清单，标出正在用的
python -m voice_agent voices zf_092                 # 试听一个
python -m voice_agent voices --audition --female --count 8   # 连着听 8 个
python -m voice_agent voices --set zf_092           # 写进配置（不动其它内容）
```

桌面界面「设置 → 语音与算力 → 音色」里也有下拉框和「试听」按钮，改完立刻生效。
默认 **zf_003**：230 Hz 的清亮女声，上面那张表里综合最干净的一档。
想更亮可以试 zf_092 / zf_074，想更沉稳试 zf_006。
...
调 length_scale 与语速能把 8.5 秒的音频压到 7.1 秒，但 RTF 始终在 0.9 上下 ——
它就是个约 1 倍实时的模型，快不了。真正管用的是**只把第一块切短**：

1. 第一块切到 16 字左右 —— 耳朵只需要等这一块；
2. **后面的块按整句走**（46 字、只认句末标点）。切得越碎，每一块都要重新起调，
   语气就越假，接缝还越多；
3. **预取线程**：播第一块的时候，后面的块已经在合成了（speak 里的 tts-prefetch），
   所以块放大不会让等待变长；
4. silence_scale 用 **1.0** —— VITS 需要 1.15 那点停顿才有「人味」，
   Kokoro 本身停顿就长，再加就拖沓。
...
tts:
  engine: kokoro                       # vits 快 / kokoro 音色多但慢（换引擎要重启）
  voice: zf_003                        # 音色写名字；见第四节「音色」那一节
  speed: 1.0
...
| 语音合成（Kokoro） | RTF 0.94（音质更好，但几乎贴着实时） |
| 语音识别 | RTF 0.03（1 秒音频约 30 毫秒） |
| 端到端响应 | 说完话到开口：规则模式约 0.1s；LLM 模式取决于模型首字延迟 |
| 打包体积 | dist/VoiceAgent 约 499 MB / 1225 个文件（模型不在包内） |

八个测试脚本共 **239 项断言全部通过**：selftest 10、test_pipeline 27、
test_llm_loop 12、test_skills 41、test_webui 55、test_gui 49、test_voices 33、
test_speaker 12。
...
Kokoro 的 0~2 号是英文音色，拿它念中文就是又闷又粗还带嘶声（第四节有实测数据）。
代码会自动把它换成中文音色，想自己挑：
...
│  ├─ speech.py          唤醒词 KWS、VAD、ASR、TTS（VITS / Kokoro）
