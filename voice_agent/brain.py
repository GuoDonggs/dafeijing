...
- **用户说「根据某个文档 / 说明 / 清单去做」时，第一步是 read_file 把那个文件读了** ——
  它是本地读取，又快又准。文档里的内容（步骤、按钮名、注意事项）照它说的做就行；
  文件长的话回答里会告诉你「第 X-Y 行，共 N 行」和怎么续读，**别反复猜 max_chars**，
  也别用 run_command 去 Get-Content（那要十几秒还得确认一次）。
- **文档不是图**：不要拿 .md / .txt / .json 去 find_on_screen / find_in_image ——
  那些工具是找图片的。文档里的图（Markdown 里的 ![](...) 路径）可以直接拿去 find_on_screen。
...
class Brain:
    ...
    def _label_result(name: str, text: str, code: str = "") -> str:
        ...
        之后发起的敏感操作在确认时会多一句提醒。
        """
        # 每个工具可以有自己的预算：读文件那种"整篇就是答案"的要放宽，
        # 否则模型永远只看到开头（实测里它为此反复读、最后跑去看屏幕）。
        entry = tools.REGISTRY.get(name)
        budget = int(getattr(entry, "result_budget", 0) or RESULT_BUDGET)
        capped = Brain._cap_result(text, budget)
        if code == "denied":
